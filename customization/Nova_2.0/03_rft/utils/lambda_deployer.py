...
def update_lambda_layers(
    ...
    This function allows you to add, update, or remove Lambda layers from an existing
    function. Layers are useful for sharing code and dependencies across multiple functions.
    
    Args:
        function_name: Name of the Lambda function to update
        layers: List of Lambda layer ARNs to attach. Pass an empty list to remove all layers.
                Example: ["arn:aws:lambda:us-east-1:123456789012:layer:my-layer:1"]
        region: AWS region (default: "us-east-1")
    
    Returns:
        dict: Updated Lambda function configuration
    ...
    try:
        # Get current function configuration to show changes
        print(f"🔄 Updating layers for Lambda function: {function_name}")
        ...
        if current_layers:
            print(f"📦 Current layers ({len(current_layers)}):")
            for layer in current_layers:
                layer_arn = layer.get('Arn', 'Unknown')
                print(f"   - {layer_arn}")
        else:
            print(f"📦 Current layers: None")
        
        # Update the function configuration with new layers
        update_params = {
            'FunctionName': function_name,
        }
        
        # Only include Layers parameter if we have layers to add
        # AWS API requires us to include the parameter even if empty to remove layers
        update_params['Layers'] = layers
        
        response = lambda_client.update_function_configuration(**update_params)
        
        # Display the updated configuration
        updated_layers = response.get('Layers', [])
        
        if layers:
            print(f"✅ Layers updated successfully! New layers ({len(layers)}):")
            for layer_arn in layers:
                print(f"   - {layer_arn}")
        else:
            print(f"✅ All layers removed successfully!")
