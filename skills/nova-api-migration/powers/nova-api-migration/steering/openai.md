...
Covers the **OpenAI API source side**. For everything on the Nova side — model IDs, inference-config tables, reasoning effort rules, tool shape, structured output, streaming, and the validation checklist — load the **`nova-target.md`** steering file (via `readSteering`). This file does not repeat them.
...
2. **OpenAI on Amazon Bedrock** — the customer already runs OpenAI models on Bedrock (GPT-5.5/5.4 via the Responses API, or gpt-oss via Converse/Responses/Chat Completions). Auth/Region/billing are already AWS-native, so this is usually lower-effort. Load the `openai-on-bedrock-patterns.md` steering file.
...
- [ ] **OpenAI on Amazon Bedrock** — an `openai` client pointed at a `bedrock-mantle` base URL, the `BedrockOpenAI` client, or `boto3` `converse`/`invoke_model` with an `openai.*` model ID. Load the `openai-on-bedrock-patterns.md` steering file for the specific paths.
...
You **MUST** load the `openai-feature-mapping.md` steering file for the complete field mapping table.
You **SHOULD** load the `openai-code-examples.md` steering file for before/after patterns.

If the source runs on **OpenAI hosted on Amazon Bedrock**, load the `openai-on-bedrock-patterns.md` steering file — the gpt-oss-on-Converse case is nearly a one-line `modelId` swap.
...
Run the validation checklist in the **nova-target.md** steering file, plus these OpenAI-specific checks:
