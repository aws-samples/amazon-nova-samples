...
Covers the **Claude API source side**. For everything on the Nova side — model IDs, inference-config tables, reasoning effort rules, tool shape, structured output, streaming, and the validation checklist — load the **`nova-target.md`** steering file (via `readSteering`). This file does not repeat them.
...
You **MUST** load the `claude-feature-mapping.md` steering file for the complete mapping table.
You **SHOULD** load the `claude-code-examples.md` steering file for before/after patterns.
...
Run the validation checklist in the **nova-target.md** steering file, plus these Claude-specific checks:
