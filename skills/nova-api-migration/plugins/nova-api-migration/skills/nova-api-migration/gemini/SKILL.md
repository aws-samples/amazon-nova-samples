---
name: nova-api-migration-gemini
description: Source guide for migrating Google Gemini Python API code to Amazon Nova 2 Lite. Covers the deprecated google-generativeai SDK, the current google-genai SDK (generateContent and Interactions API). Pairs with ../references/nova-target.md for the Nova side.
tags: [skill, migration, gemini, google, api, nova, bedrock]
---

# Gemini → Nova 2 Lite (API Source Guide)

Covers the **Gemini API source side**. For everything on the Nova side — model IDs, inference-config tables, reasoning effort rules, tool shape, structured output, streaming, and the validation checklist — read **`../references/nova-target.md`**. This file does not repeat them.
...
If the source uses the deprecated `google-generativeai` SDK or the Interactions API, also read `references/generate-content-patterns.md` for parameter mappings.
...
Run the **nova-target.md validation checklist**, plus these Gemini-specific checks:
