...
class AppointmentService {
    ...
    formatAvailabilityCalendar(availability) {
    ...
    formatAppointmentsCalendar(appointments) {
    ...
    getDoctorsBySpecialty(specialty) {
        return this.doctors.filter(doc => doc.specialty.toLowerCase() === specialty.toLowerCase()).map(({ id, name, specialty }) => ({ id, name, specialty }));
    }
    ...
    getPatientAppointments(patientName) {
        return this.appointments.filter(apt => apt.patientName.toLowerCase().includes(patientName.toLowerCase()));
    }
    ...
    createAppointment(doctorId, patientName, date, time, reason) {
        ...
        const conflictingAppointment = this.appointments.find(apt => apt.doctorId === doctorId && apt.date === date && apt.time === time);
