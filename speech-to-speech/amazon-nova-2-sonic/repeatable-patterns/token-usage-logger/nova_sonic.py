...
class BedrockStreamManager:
                        ...
                        try:
                            ...
                            if 'event' in json_data:
                                if 'completionStart' in json_data['event']:
                                    debug_print("Completion start detected")
                                    ...
                                    if not self.display_assistant_text:
                                        self.chat_history.add_message(role, text_content)
                                    ...
                                    usage_data = json_data['event']['usageEvent']
                                    debug_print(f"Usage event received: {usage_data}")
                                    # Log usage event to chat history
                                    self.chat_history.add_usage_event(
                                        completion_id=usage_data.get('completionId', ''),
                                        session_id=usage_data.get('sessionId', ''),
                                        prompt_name=usage_data.get('promptName', ''),
                                        total_input_tokens=usage_data.get('totalInputTokens', 0),
                                        total_output_tokens=usage_data.get('totalOutputTokens', 0),
                                        total_tokens=usage_data.get('totalTokens', 0),
                                        details=usage_data.get('details', {})

                                    )
                                    debug_print(f"Usage metrics - Input: {usage_data.get('totalInputTokens', 0)}, Output: {usage_data.get('totalOutputTokens', 0)}, Total: {usage_data.get('totalTokens', 0)}")
    ...
    async def save_chat_history_to_file(self, directory="chat_histories"):
        """Save the current chat history to a timestamped file"""
        import os
        ...
        # Print summary of the conversation including usage metrics
        usage_summary = self.chat_history.get_token_usage_summary()
        print(f"Chat history saved to {filepath}")
        print(f"Usage Summary: {json.dumps(usage_summary, indent=4)}")
    ...
    async def close(self):
        ...
        self.is_active = False
        await self.save_chat_history_to_file()
