...
class ToolProcessor:
    ...
    async def _run_tool(self, tool_name, tool_content):
        ...
        if tool == "getdateandtimetool":
            ...
            print(f"datetime tool result: {date_time}")
            return date_time
        else:
            return {
...
class BedrockStreamManager:
    ...
    def __init__(self, model_id='amazon.nova-2-sonic-v1:0', region='us-east-1', session_id=None):
        """Initialize the stream manager."""
        self.model_id = model_id
        self.region = region
        self.session_id = session_id or str(uuid.uuid4())[:8]  # Use first 8 chars
        ...
        self.should_print_text = True  # Control whether this session should print text
        self.role = None
        ...
        self.model_session_id = None  # Will be set from completionStart event
        self.toolUseContent = ""
        self.toolUseId = ""
        self.toolName = ""
    ...
    async def initialize_stream(self):
        ...
        try:
            ...
            self.is_active = True
            default_system_prompt = "You are a friend. The user and you will engage in a spoken dialog exchanging the transcripts of a natural real-time conversation. Help the user to tell stories."
        ...
        try:
            while self.is_active:
                try:
                    ...
                    # Future was cancelled during shutdown - this is expected
                    # Happens when AWS CRT tries to deliver data to cancelled futures
                    debug_print(f"Future cancelled during receive (expected during shutdown): {e}")
                    break
                ...
                try:
                    if result.value and result.value.bytes_:
                        try:
                            ...
                            # Check if stream was force-stopped before processing event
                            if not self.is_active:
                                debug_print("Stream was stopped, ignoring event")
                                break

                            # Handle different response types
                            if 'event' in json_data:
                                if 'completionStart' in json_data['event']:
                                    # Capture model's session ID
                                    completion_start = json_data['event']['completionStart']
                                    self.model_session_id = completion_start.get('sessionId', '')[:8]  # First 8 chars
                                    print(f"completionStart: {json_data['event']}")
                                    ...
                                    # Only print if this session is active (should_print_text=True)
                                    if self.should_print_text:
                                        # Include model session ID if available
                                        session_label = f"{self.session_id}/{self.model_session_id}" if self.model_session_id else self.session_id
                                        if (self.role == "ASSISTANT" and self.display_assistant_text):
                                            print(f"[{session_label}] Assistant: {text_content}")
                                        elif (self.role == "USER"):
                                            print(f"[{session_label}] User: {text_content}")
                                elif 'audioOutput' in json_data['event']:
                                    # Don't queue audio if stream was stopped or barge-in detected
                                    if not self.is_active or self.barge_in:
                                        break
                            ...
                            # Only if stream is still active
                            if self.is_active:
                    ...
                    debug_print("Response processing cancelled (shutdown)")
                    break
                except Exception as e:
                    ...
                    elif "Invalid input request" in str(e) and not self.is_active:
                        # Stream was force-stopped - this is expected, don't log
                        pass
                    elif "InvalidStateError" in str(e) or "CANCELLED" in str(e):
                        # Future was cancelled during shutdown - this is expected
                        debug_print(f"Future cancelled during shutdown: {e}")
                        break
                    else:
            ...
            if ("InvalidStateError" not in str(e) and
                "CANCELLED" not in str(e) and
                "Invalid input request" not in str(e)):
    ...
    def force_stop(self):
        """Immediately stop processing events (for emergency shutdown)"""
        debug_print("Force stopping stream")
        self.is_active = False
        self.should_print_text = False

        # Clear all queues to prevent processing stale events
    ...
    async def close(self):
        ...
        1. Send close events to server
        2. Close the stream (stops data flow)
        ...
        # STEP 1: Send close events to server
        await self.send_audio_content_end_event()
        await self.send_prompt_end_event()
        await self.send_session_end_event()

        # STEP 2: Close the stream (stops receiving data from AWS)
        ...
        # STEP 3: Wait for pending operations to complete
        # Give the AWS CRT callbacks time to finish delivering any in-flight data
        await asyncio.sleep(0.1)

        # STEP 4: Now safe to cancel tasks (no more data coming)
        ...
        for task in self.pending_tool_tasks.values():
            if not task.done():
                ...
                try:
                    ...
                    debug_print(f"Error cancelling tool task: {e}")
