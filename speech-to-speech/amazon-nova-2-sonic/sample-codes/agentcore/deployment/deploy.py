...
Examples:
    python deploy.py sonic
    python deploy.py strands --region us-west-2
    python deploy.py langchain --agent-name my-langchain-agent
    python deploy.py sonic --agent-name my-sonic-agent
"""
...
class AgentCoreDeployer:
    ...
    def __init__(self, websocket_folder: str, args: argparse.Namespace):
        ...
        self.agent_name = args.agent_name or f"bidi_{websocket_folder}_agent"
    ...
    def deploy_mcp_gateway(self) -> Optional[Dict]:
        ...
        if self.websocket_folder not in ('strands', 'langchain'):
            return None
    ...
    def deploy_agent(self, role_arn: str, gateway_info: Optional[Dict] = None) -> Dict:
        ...
        try:
            ...
            if self.websocket_folder in ('strands', 'langchain') and gateway_info:
    ...
    def print_summary(self, deployment_info: Dict):
        ...
        print(f"   ./deployment/start_client.sh {self.websocket_folder}")
        ...
        print(f"   python deployment/cleanup.py {self.websocket_folder}")
    ...
    def deploy(self):
        ...
        try:
            ...
            # Step 3: Deploy agent
            deployment_info = self.deploy_agent(role_arn, gateway_info)
...
Examples:
  ...
  python deploy.py strands --region us-west-2
  python deploy.py langchain --agent-name my-langchain-agent
  python deploy.py sonic --agent-name my-sonic-agent --local-build
...
Environment Variables:
  ...
  AWS_REGION    AWS Region (default: us-east-1)
    ...
    parser.add_argument(
        'websocket_folder',
        choices=['sonic', 'strands', 'langchain', 'echo'],
        help='Websocket folder to deploy (sonic, strands, langchain, or echo)'
    )
