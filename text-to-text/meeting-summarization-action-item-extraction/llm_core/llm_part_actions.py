...
from llm_core.llm_prompt_bank import (one_stage_participant_action_prompt,
                             two_stage_partial_participant_action_generation_prompt, 
                             two_stage_partial_participant_action_aggregation_prompt
                            )

from llm_core.llm_prompt_bank import (one_stage_participant_action_system_prompt_v2,
                                      one_stage_participant_action_usr_prompt_v2,
                                      participant_action_assistant_prompt)


def generate_person_action_item( transcript: str,                          
                                max_tokens=2000, 
                                temp=0, 
                                topK=50, 
                                ...
                                text_only=True ):
    '''
    Generate participant-level action items.
    '''
   
    usr_msg = one_stage_participant_action_prompt.format(meeting_transcript = transcript)
...
def generate_person_action_item_v2( transcript: str,                          
                                max_tokens=2000, 
                                temp=0, 
                                topK=50, 
                                stop_sequences=["Human:", "User:"], 
    ...
    usr_msg = one_stage_participant_action_usr_prompt_v2.format(meeting_transcript = transcript)
    
    response = get_bedrock_response(usr_msg,  
                                    system=one_stage_participant_action_system_prompt_v2,
                                    assistant_message=participant_action_assistant_prompt,
                                    max_tokens=max_tokens,
                                    temp=temp,
                                    topK=topK, 
                                    stop_sequences=stop_sequences, 
...
def generate_participant_actions_two_stages(transcript, 
                                            model_id, 
                                            temperature, 
                                            n_chapters):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        chapter_results = list(executor.map(get_action_items_stage_1, 
                                            [two_stage_partial_participant_action_generation_prompt] * n_chapters,
    ...
    partial_agg_prompt = two_stage_partial_participant_action_aggregation_prompt.format(meeting_action_items = stage1_llm_response)
...
# temp = 0.2
# stage1_llm_response, stage2_llm_response, total_latency, total_cost = generate_participant_actions_two_stages(transcript, model_id, temp, n_chapters)  
